...
namespace bluevoxel
{
    ...
    static std::shared_ptr<h2o::gfx::Texture> get_selection_texture(
        ...
        const h2o::UncookedBlockModel::TriangleHandle& triangle_handle)
    {
        const u32 texture_id = block_type.texture_ids[triangle_handle.face_handle.face_index];
    ...
    bool UVEditor::update(
        ...
        const h2o::BlockType& block_type,
        const h2o::UncookedBlockModel::TriangleHandle& triangle_handle,
        const std::unordered_set<u32>& vertex_indices)
    {
        if (!ImGui::CollapsingHeader("UV Editor", ImGuiTreeNodeFlags_DefaultOpen))
            return false;

        const auto tex = get_selection_texture(voxel_pack, block_type, triangle_handle);
        if (!tex)
            return false;
        ...
        if (const auto triangle = block_model.get_triangle(triangle_handle))
        {
            for (u32 i = 0; i < 3; i++)
            {
        ...
        for (u32 i = 0; i < 3; i++)
        {
            ...
            const ImColor point_color = vertex_indices.contains(i) ?
